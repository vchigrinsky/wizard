...
Поэтому я прогнал один и тот же набор проб из трёх разных сетей подряд.

### Три сети: корпоративная, Билайн, МТС

| | корпоративная | Билайн, AS3216 | МТС, AS8359 |
| --- | --- | --- | --- |
| `youtube.com` | не разрешается в адрес (DNS) | режется по имени в рукопожатии | режется по имени в рукопожатии |
| `1.1.1.1` | мёртв полностью | работает | работает |
| `telegram.org` | открывается | не отвечает по TCP (по адресу) | не отвечает по TCP (по адресу) |

Первый вывод — не про то, **что** блокируют, а про то, **чем**. Одни и те же цели закрыты на трёх разных уровнях: DNS, имя в рукопожатии, маршрутизация. Отсюда практическое следствие: **фраза «в России заблокировано то-то» бессмысленна без указания оператора.** Схема обхода может работать у одного и не работать у другого не потому, что криво настроена, а потому что там закрыто на другом уровне — и лечится другим.

Второй вывод: два мобильных оператора ведут себя **почти одинаково**. Это, скорее всего, и означает централизованное управление правилами.

### Ошибка с регистром — не курьёз

Ошибку с регистром я нашёл на точечной проверке одного адреса и считал частным случаем. Проверил на **настоящей, всем известной блокировке YouTube** — сначала у Билайна, потом у МТС. Результат совпал:

| Имя в рукопожатии | Результат |
| --- | --- |
| `youtube.com` | дроп |
| `YOUTUBE.COM` | **работает**, отвечает настоящий YouTube |
| `www.youtube.com` | дроп |
| `WWW.YOUTUBE.COM` | **работает** |
| `YouTube.Com` | **работает** — даже вперемешку |
| `youtube.com.` (с точкой) | дроп |

**Дефект сидит в основном механизме блокировок**, а не в экспериментальной проверке. Заглавные буквы открывают заблокированный YouTube напрямую, без всякого ВПН, у двух крупнейших мобильных операторов.

При этом точка в конце имени на Selectel проходила, а у операторов не проходит. Значит коробки **разные** — производителей или версий, — а баг **общий**. Так бывает, когда все берут одну библиотеку разбора или одинаково неправильно читают стандарт.

Пользоваться этим как решением нельзя: чинится одной строкой. Но масштаб дефекта показывает, насколько эта индустрия сделана на коленке.

### Замедление: третье действие, пойманное вживую

В начале раздела я перечислил три действия — дроп, сброс и замедление. Первые два поймал сразу, третье до сих пор оставалось теорией. На МТС оно нашлось.

Узел в Узбекистане: туннель поднимается, страна подтверждается, а скорость — **0.5 Мбит/с**, три прогона подряд с одинаковым результатом. Проверил вне туннеля, сырым TCP, с разными именами в рукопожатии, в ту же минуту:

| Узел | Скорость |
| --- | --- |
| Узбекистан | **0.6 Мбит/с** |
| Казахстан | 23 Мбит/с |
| Нидерланды | 30 Мбит/с |

Замедление привязано **к адресу назначения**, а не к протоколу, имени или туннелю: душат всё, что идёт на этот адрес. На Билайне в тот же день этот же узел работал на полной скорости канала — то есть правило операторское, а не общее.

Это самое неприятное из трёх действий с точки зрения диагностики: ничего не сломано, всё «работает», просто пользоваться невозможно. И на глаз неотличимо от «плохой сети».

### Чего в обычной фильтрации нет

Заодно проверил две популярные версии — обе не подтвердились **ни на одной из сетей**:

- **Порога по параллельным рукопожатиям нет.** Пачки по 2, 4, 8 и 16 одновременных соединений к одному адресу — одиночная проба сразу после каждой проходит всегда. Никакой «заморозки». Потери внутри больших пачек объясняются загруженностью канала.
- **Отсечки по объёму нет.** Мегабайты через флагнутый узел проходят целиком, на скорости контрольного. Значит механика «16 килобайт и тишина» — принадлежность **режима белых списков**, а не обычной фильтрации.

### Практический итог

Из шести прямых подключений живы четыре на Билайне и три на МТС. Узел в Амстердаме падает на обеих сетях по описанной причине: его ключ ходит с именем `www.google.com`, а к этому адресу оно не подходит.

Тот же узел, тот же ключ, но имя заглавными буквами — на МТС дал **45 Мбит/с**, быстрее всех остальных маршрутов. Заплатка временная, но механизм она подтверждает на трёх независимых сетях.

Остаются незакрытыми домашний вайфай, отельный, у друзей и — главное — мобильный интернет в области, там, где включают белые списки. Методика и инструменты в разделе 5.
