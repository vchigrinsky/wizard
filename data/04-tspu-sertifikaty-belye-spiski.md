...
Поэтому нужна серия замеров из разных сетей. Первая точка сравнения уже есть.

### Вторая сеть: Билайн, аэропорт

Тот же набор проб, прогнанный из вайфая аэропорта — Вымпелком, AS3216, Москва. И картина отличается принципиально.

**Одно и то же закрыто разными средствами.**

| | корпоративная сеть | Билайн |
| --- | --- | --- |
| `youtube.com` | не разрешается в адрес (DNS) | разрешается, режется по имени в рукопожатии |
| `1.1.1.1` | мёртв полностью | работает |
| `telegram.org` | открывается | не отвечает по TCP (закрыт по адресу) |

Три разных уровня — DNS, SNI и маршрутизация — на одни и те же цели у разных операторов. Это и есть главный практический вывод: **утверждение «в России заблокировано то-то» бессмысленно без указания оператора.** Одна и та же схема обхода может работать у одного и не работать у другого не потому, что «плохо настроено», а потому что закрыто на другом уровне.

**А теперь самое интересное.** Помните ошибку с регистром из моего первого замера? Я нашёл её на точечно включённой проверке одного адреса и считал курьёзом. Проверил на **настоящей, всем известной блокировке YouTube** у мобильного оператора:
...
То есть **тот же самый дефект сидит в основном механизме блокировок**, а не в какой-то экспериментальной проверке. Заглавные буквы открывают заблокированный YouTube напрямую, без всякого ВПН.

Обратите внимание на различие: точка в конце имени на Selectel проходила, а здесь не проходит. Значит это разные коробки — разных производителей или версий — но **общий баг у обеих**. Такое бывает, когда все берут одну и ту же библиотеку разбора или одинаково неправильно читают стандарт.

Пользоваться этим как решением, разумеется, нельзя: чинится одной строкой. Но масштаб дефекта показывает, насколько эта индустрия сделана на коленке.

**Чего здесь нет.** Заодно проверил две популярные версии, и обе не подтвердились:

- **Порога по параллельным рукопожатиям нет.** Пачки по 2, 4, 8 и 16 одновременных соединений к одному адресу — одиночная проба сразу после каждой пачки проходит всегда. Никакой «заморозки» не наступает. Потери внутри больших пачек объясняются загруженностью самого вайфая, а не фильтром.
- **Отсечки по объёму нет.** Два мегабайта через флагнутый узел проходят целиком, на той же скорости, что и через чистый. Значит механика «16 килобайт и тишина» — принадлежность режима белых списков, а не обычной фильтрации.

**Практический итог для нашей схемы.** Из шести прямых подключений на этой сети живы четыре. Узел в Амстердаме лёг ровно по описанной причине — его ключ ходит с именем `www.google.com`, а оно к этому адресу не подходит. Тот же самый узел с тем же ключом, но именем заглавными буквами — работает на полной скорости сети.

Методика и инструменты, которыми это снималось, — в разделе 5. Остаются незакрытыми домашний вайфай, отельный, у друзей и мобильный интернет в области — там, где включают белые списки.
